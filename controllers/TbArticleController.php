...
use app\models\TbArticleSearch;
...
/**
 * TbArticleController implements the CRUD actions for TbArticle model.
 */
class TbArticleController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $comment->type_collection = 4;
        $allComment = TbComment::find()->where(['type_collection'=>4, 'id_collection'=>$model->id_article])->orderBy('id_comment desc')->all();
  ...
  //           //$path = Parameter::find()->where(['keys'=>'url_article'])->one()->value . $model->attachment;
  ...
  //       if ($model->load(Yii::$app->request->post())) {
  //           $name = TbArticle::findOne($model->id_article);
  //           $name = $model;
        //  //var_dump(isset($_FILES['TbArticle']['name']['attachment']));die();
        //  // if($_FILES['TbArticle']['name']['attachment'] != ''){
        //  //  $attachment = UploadedFile::getInstance($model, 'attachment');
  ...
  //       }
        ...
        $cur = $model->attachment;
        if ($model->load(Yii::$app->request->post())) {
            $name   = TbArticle::findOne($id);
            $link   = Parameter::find()->where(['keys'=>'url_article'])->one()->value;
    ...
    {
        $name   = TbArticle::findOne($id);
        $unlink = Parameter::find()->where(['keys'=>'url_article'])->one()->value.$name->attachment;
        if(!empty($name->attachment)){
            @unlink($unlink);
        }
        ...
        if(Yii::$app->user->isGuest != ""){
            ...
            if ($extention[1] == 'pdf' || $extention[1] == 'PDF'){
              ...
              if(count($data) < 10){               
                  ...
                  if($model->save()){
                    $model = $this->findModel($id);
                    $link = Parameter::find()->where(['keys'=>'library_url'])->one()->value.'Attachment_article/'.$model->attachment;
                   $this->redirect($link);
                  }else{

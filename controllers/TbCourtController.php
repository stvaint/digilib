...
/**
 * TbCourtController implements the CRUD actions for TbCourt model.
 */
class TbCourtController extends Controller
{
    ...
    /**
     * Lists all TbCourt models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new TbCourtSearch;
    ...
    /**
     * Displays a single TbCourt model.
     * @param integer $id
     * @return mixed
     */
    public function actionView($id)
    {
        ...
        $comment->type_collection = 3;
        $allComment = TbComment::find()->where(['type_collection'=>3, 'id_collection'=>$model->id_court])->orderBy('id_comment desc')->all();
        if ($comment->load(Yii::$app->request->post()) && $comment->save()) {
            return $this->redirect(['view', 'id' => $model->id_court]);
        } else {
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if(!empty($attachment->name)){
                ...
                $path              = Parameter::find()->where(['keys'=>'url_court'])->one()->value . $model->attachment;
    ...
    /**
     * Updates an existing TbCourt model.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $model->reference = implode(',', $model->reference);
			      $name   = TbCourt::findOne($id);
            $link   = Parameter::find()->where(['keys'=>'url_court'])->one()->value;
            $unlink = $link.$name->attachment;
            if($_FILES['TbCourt']['name']['attachment'] != ''){
    ...
    /**
     * Deletes an existing TbCourt model.
    ...
    /**
     * Finds the TbCourt model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return TbCourt the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TbCourt::findOne($id)) !== null) {
            return $model;
        } else {
        ...
        if(Yii::$app->user->isGuest != ""){
            ...
            if ($extention[1] == 'pdf' || $extention[1] == 'PDF'){
              ...
              if(count($data) < 10){               
                  ...
                  if($model->save()){
                    $model = $this->findModel($id);
                    $link = Parameter::find()->where(['keys'=>'library_url'])->one()->value.'Attachment_court/'.$model->attachment;
                   $this->redirect($link);
                  }else{
    ...
    public function actionAttachment($id=null){
      ...
      $model->attachment = "";
      $model->save();
      return $this->redirect(['update', 'id' => $model->id_court]);
  }
}
